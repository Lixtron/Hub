...
local function _sd(fn)
	return function(...)
		local args = {...}
		task.spawn(function() fn(table.unpack(args)) end)
	end
end
...
local function GetGui()
	...
	local parentCandidates = {
		...
		{ "CoreGui",        function() return safeGetCoreGui() end },
		{ "PlayerGui",      function()
			local pg = LocalPlayer:FindFirstChildOfClass("PlayerGui")
			if not pg then pcall(function() pg = LocalPlayer:WaitForChild("PlayerGui", 5) end) end
			return pg
		end },
	}
	...
	for _ = 1, 30 do
		for _, candidate in ipairs(parentCandidates) do
			if trySetParent(candidate[2]) then
				attached = true
				break
			end
		end
	...
	newGui:GetPropertyChangedSignal("Parent"):Connect(function()
		if newGui.Parent == nil and not unloaded then
			for _, candidate in ipairs(parentCandidates) do
				if trySetParent(candidate[2]) then break end
			end
		end
	end)
...
function MacLib:Window(Settings)
	...
	base.Size = Settings.Size or UDim2.fromOffset(740, 560)

	local baseUIScale = Instance.new("UIScale")
	baseUIScale.Name = "BaseUIScale"
	baseUIScale.Parent = base
	local _appliedScale = 1
	...
	searchContainer.Size = UDim2.fromOffset(168, 28)
	searchContainer.Parent = elements

	local searchContainerUICorner = Instance.new("UICorner")
	searchContainerUICorner.CornerRadius = UDim.new(0, 8)
	...
	searchContainerUIStroke.Transparency = 0.82
	searchContainerUIStroke.Parent = searchContainer

	local searchIcon = Instance.new("ImageLabel")
	searchIcon.Image = assets.searchIcon
	searchIcon.ImageColor3 = Color3.fromRGB(158, 158, 158)
	...
	searchInput.PlaceholderColor3 = Color3.fromRGB(112, 112, 112)
	searchInput.Text = ""
	searchInput.TextColor3 = Color3.fromRGB(218, 218, 218)
	...
	searchInput.Position = UDim2.new(0, 27, 0.5, 0)
	...
	searchResults.Size = UDim2.fromOffset(215, 0)
	...
	searchResultsUICorner.CornerRadius = UDim.new(0, 10)
	searchResultsUICorner.Parent = searchResults
	...
	searchResultsUIStroke.Transparency = 0.84
	searchResultsUIStroke.Parent = searchResults

	local searchResultsUIPadding = Instance.new("UIPadding")
	searchResultsUIPadding.PaddingTop = UDim.new(0, 8)
	searchResultsUIPadding.PaddingBottom = UDim.new(0, 8)
	searchResultsUIPadding.Parent = searchResults
	...
	searchEmptyLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium, Enum.FontStyle.Normal)
	searchEmptyLabel.Text = "No results found"
	searchEmptyLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
	searchEmptyLabel.TextSize = 13
	searchEmptyLabel.TextTransparency = 0.55
	searchEmptyLabel.TextXAlignment = Enum.TextXAlignment.Center
	searchEmptyLabel.BackgroundTransparency = 1
	searchEmptyLabel.BorderSizePixel = 0
	searchEmptyLabel.Size = UDim2.new(1, 0, 0, 30)
	searchEmptyLabel.ZIndex = 51
	...
	local function restoreHighlights()
		for _, h in pairs(flashHighlights) do
			if h.obj and h.obj.Parent then
				h.obj.TextTransparency = h.origTransparency
	...
	local function clearResultEntries()
	...
	local function navigateTo(entry)
		...
		if entry.scrollFrame and entry.element then
			...
			local relY = elem.AbsolutePosition.Y - sf.AbsolutePosition.Y + sf.CanvasPosition.Y
			local targetY = math.clamp(relY - sf.AbsoluteSize.Y / 3, 0, math.max(0, sf.AbsoluteCanvasSize.Y - sf.AbsoluteSize.Y))
			sf.CanvasPosition = Vector2.new(0, targetY)

			activeFlashId = activeFlashId + 1
			local thisFlash = activeFlashId
			restoreHighlights()
			...
			for _, child in pairs(elem:GetDescendants()) do
				if (child:IsA("TextLabel") or child:IsA("TextButton")) and child.TextTransparency < 0.9 then
					table.insert(toFlash, {obj = child, origTransparency = child.TextTransparency, origColor = child.TextColor3})
				end
			end
			flashHighlights = toFlash

			local function setFlash(bright)
				if thisFlash ~= activeFlashId then return end
				for _, h in pairs(toFlash) do
					if h.obj and h.obj.Parent then
						h.obj.TextTransparency = bright and 0 or h.origTransparency
						h.obj.TextColor3 = bright and Color3.fromRGB(195, 220, 255) or h.origColor
					end
				end
			end
			...
			task.spawn(function()
				...
				if thisFlash == activeFlashId then
					restoreHighlights()
				end
			end)
		end
	...
	local function runSearch(query)
		...
		for _, entry in ipairs(searchRegistry) do
			if tostring(entry.name):lower():find(q, 1, true) then
		...
		for i, entry in ipairs(matches) do
			...
			btnPadding.PaddingTop = UDim.new(0, 7)
			btnPadding.PaddingBottom = UDim.new(0, 7)
			btnPadding.Parent = btn

			local label = Instance.new("TextLabel")
			label.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium, Enum.FontStyle.Normal)
			label.Text = tostring(entry.name) .. "  ·  " .. tostring(entry.tabName or "")
		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					registerElement(Settings.Name, button)
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					registerElement(Settings.Name, toggle)
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					sliderName.Size = UDim2.new(0.45, 0, 0, 0)
					...
					registerElement(Settings.Name, slider)
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					registerElement(Settings.Name, input)
				...
				function SectionFunctions:Keybind(Settings, Flag)
					...
					registerElement(Settings.Name, keybind)
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					dropdownName.TextTruncate = Enum.TextTruncate.None
					...
					local function CalculateDropdownSize()
						...
						local spacing = dropdownFrameUIListLayout.Padding.Offset * math.max(visibleCount - 1, 0)
						local listHeight = math.min(totalHeight + spacing + padding, MAX_LIST_H)
						return HEADER_H() + listHeight
					end
					...
					local function ToggleDropdown()
						...
						local targetSize = isDropdownOpen and UDim2.new(1, 0, 0, CalculateDropdownSize()) or UDim2.new(1, 0, 0, HEADER_H())
					...
					registerElement(Settings.Name, dropdown)
				...
				function SectionFunctions:Colorpicker(Settings, Flag)
					...
					registerElement(Settings.Name, colorpicker)
			...
			tabContext.selectFunc = SelectCurrentTab
...
function WindowFunctions:SetKeyTimer(expiresAt, tier)
	...
	local function parseISO(value)
		...
		return os.time({
			...
			hour = tonumber(hour),
			min = tonumber(minute),
			sec = math.floor(tonumber(second))
		})
	end
	...
	elseif type(expiresAt) == "string" then
		expiry = parseCompact(expiresAt) or parseISO(expiresAt)
	end
	...
	local function updateTimer()
		...
		local currentSecond = math.floor(remaining)

		if currentSecond ~= lastDisplayedSecond then
			lastDisplayedSecond = currentSecond
			keyTimerLabel.Text = formatRemaining(remaining)
		end

		if remaining <= 0 and _keyTimerConn then
			_keyTimerConn:Disconnect()
			_keyTimerConn = nil
		end
	end
